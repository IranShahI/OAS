                            ...
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                @foreach($reports as $report)
                                    <tr class="gradeX">
                                        ...
                                        <td style="text-align: center">@if($report->isConfirmed === 1)
                                                <a href="/staff/unConfirm/{{$report->id}}" class="btn btn-danger">عدم تایید</a>
                                            @else
                                                <a href="/staff/confirm/{{$report->id}}" class="btn btn-success">تایید کردن</a>
                                                @endif

                                        </td>
                                        <td style="text-align: center"><a title="جزییات" href="/staff/confirmation/edit/{{$report->customer->customerCode}}" class="user-edit-icon"><span class="fa fa-trash"></span></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
